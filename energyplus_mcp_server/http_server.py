...
# =============================================================================
# Configuration Endpoints (for n8n dynamic URL handling)
# =============================================================================

# Path to the ngrok config file
NGROK_CONFIG_FILE = Path(config.paths.output_dir).parent / "config" / "ngrok_url.txt"


@app.get("/api/config/ngrok-url")
async def get_ngrok_url():
    """
    Get the current ngrok URL from config file.

    This endpoint allows n8n workflows to dynamically fetch the current
    ngrok URL without hardcoding it. Update the config file when ngrok restarts.

    Config file location: {project_root}/config/ngrok_url.txt
    """
    try:
        if NGROK_CONFIG_FILE.exists():
            url = NGROK_CONFIG_FILE.read_text().strip()
            return {
                "success": True,
                "ngrok_url": url,
                "config_file": str(NGROK_CONFIG_FILE),
                "last_updated": datetime.fromtimestamp(
                    NGROK_CONFIG_FILE.stat().st_mtime
                ).isoformat()
            }
        else:
            # Try to get from local ngrok API
            try:
                ...
                with urllib.request.urlopen("http://127.0.0.1:4040/api/tunnels", timeout=2) as response:
                    ...
                    if data.get("tunnels"):
                        ...
                        return {
                            ...
                            "ngrok_url": url,
                            "source": "ngrok_api",
                            "note": "Auto-detected from running ngrok instance"
                        }
            except Exception:
                pass
            ...
            return {
                ...
                "config_file": str(NGROK_CONFIG_FILE),
                "instructions": "Create the config file with your ngrok URL, or ensure ngrok is running locally"
            }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/config/ngrok-url")
async def set_ngrok_url(url: str):
    """
    Update the ngrok URL in the config file.

    This can be called when ngrok restarts with a new URL.
    """
    try:
        # Ensure config directory exists
        NGROK_CONFIG_FILE.parent.mkdir(parents=True, exist_ok=True)
        ...
        NGROK_CONFIG_FILE.write_text(url)
